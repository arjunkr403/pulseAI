...
class ChatResponse(BaseModel): # Defines what your AI chatbot returns.
    ...
    thread_id: str # Returns the conversation ID so the frontend can continue the same chat.

class ApprovalRequest(BaseModel): # Defines what the frontend sends when a user approves or rejects an AI recommendation.
    thread_id: str
    approved: bool

class IncidentResponse(BaseModel): # Defines the response returned when fetching incident records from the database.
    id: int
    cpu_usage: float
    rps: float
    latency_p95: float
